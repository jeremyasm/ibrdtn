...
#include "routing/newflooding/NewFloodRoutingExtension.h"
...
namespace dtn
{
	namespace routing
	{
		const std::string NewFloodRoutingExtension::TAG = "NewFloodRoutingExtension";

		NewFloodRoutingExtension::NewFloodRoutingExtension()
		{
			// write something to the syslog
			IBRCOMMON_LOGGER_TAG(NewFloodRoutingExtension::TAG, info) << "Initializing new flooding routing module" << IBRCOMMON_LOGGER_ENDL;
		}
		...
		void NewFloodRoutingExtension::eventDataChanged(const dtn::data::EID &peer) throw ()
		{
			// transfer the next bundle to this destination
			_taskqueue.push( new SearchNextBundleTask( peer ) );
		}

		void NewFloodRoutingExtension::eventBundleQueued(const dtn::data::EID &peer, const dtn::data::MetaBundle &meta) throw ()
		{
		...
		void NewFloodRoutingExtension::componentUp() throw ()
		{
			...
			try {
				...
				IBRCOMMON_LOGGER_TAG(NewFloodRoutingExtension::TAG, error) << "componentUp failed: " << ex.what() << IBRCOMMON_LOGGER_ENDL;
			}
		}

		void NewFloodRoutingExtension::componentDown() throw ()
		{
			try {
				// stop the thread
				stop();
				...
				IBRCOMMON_LOGGER_TAG(NewFloodRoutingExtension::TAG, error) << "componentDown failed: " << ex.what() << IBRCOMMON_LOGGER_ENDL;
			}
		}

		const std::string NewFloodRoutingExtension::getTag() const throw ()
		{
			return "new flooding";
		}

		void NewFloodRoutingExtension::__cancellation() throw ()
		{
			_taskqueue.abort();
		}

		void NewFloodRoutingExtension::run() throw ()
		{
			...
			while (true)
			{
				try {
					...
					IBRCOMMON_LOGGER_DEBUG_TAG(NewFloodRoutingExtension::TAG, 50) << "processing task " << t->toString() << IBRCOMMON_LOGGER_ENDL;
					...
					try {
						try {
							...
							// lock the neighbor database while searching for bundles
							{
								...
								// some debug
								IBRCOMMON_LOGGER_DEBUG_TAG(NewFloodRoutingExtension::TAG, 40) << "search some bundles not known by " << task.eid.getString() << IBRCOMMON_LOGGER_ENDL;
						...
						IBRCOMMON_LOGGER_DEBUG_TAG(NewFloodRoutingExtension::TAG, 20) << "task failed: " << ex.what() << IBRCOMMON_LOGGER_ENDL;
					}
				} catch (const std::exception &ex) {
					IBRCOMMON_LOGGER_DEBUG_TAG(NewFloodRoutingExtension::TAG, 15) << "terminated due to " << ex.what() << IBRCOMMON_LOGGER_ENDL;
					return;
				}
		...
		NewFloodRoutingExtension::SearchNextBundleTask::SearchNextBundleTask(const dtn::data::EID &e)
		 : eid(e)
		{ }

		NewFloodRoutingExtension::SearchNextBundleTask::~SearchNextBundleTask()
		{ }

		std::string NewFloodRoutingExtension::SearchNextBundleTask::toString()
		{
